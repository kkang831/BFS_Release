# Modified from Omnieraser's pipeline 
# https://github.com/PRIS-CV/Omnieraser/blob/main/pipeline_flux_control_removal.py
...
class FluxControlPipeline(
    ...
    def prepare_image_with_mask(
        ...
        # 5.resize mask to latents shape we concatenate the mask to the latents (from flux_fill pipeline)
        height = 2 * (int(height) // (self.vae_scale_factor * 2))
        width = 2 * (int(width) // (self.vae_scale_factor * 2))
        
        masks = masks[:, 0, :, :]  # batch_size, 8 * height, 8 * width (mask has not been 8x compressed)
        masks = masks.view(
        ...
        packed_control_image = torch.cat([image_latents, masks], dim=-1) # bs, h*w, 64+64=128
    ...
    def __call__(
        ...
        max_sequence_length: int = 512,
        trans_vae: Optional[AutoencoderKL] = None,
    ):
        r"""
        ...
        if len(prompt) == batch_size * 2:
            ...
            (
                ...
                prompt=prompt*2 if prompt is not None else prompt,
                prompt_2=prompt_2*2 if prompt_2 is not None else prompt_2,
                prompt_embeds=prompt_embeds,
                pooled_prompt_embeds=pooled_prompt_embeds,
                device=device,
                num_images_per_prompt=num_images_per_prompt,
            ...
            raise ValueError(
                f"Prompt length {len(prompt)} does not match batch size {batch_size}. Please ensure that the prompt"
        ...
        control_image, _, _ = self.prepare_image_with_mask(
            image=control_image,
            mask=control_mask,
            width=width,
            height=height,
            batch_size=batch_size * num_images_per_prompt * 2, 
            num_images_per_prompt=num_images_per_prompt,
            device=device,
        ...
        if output_type == "latent":
            ...
            composite_latents, foreground_latents = latents.chunk(2, dim=0)
            composite_latents = (composite_latents / self.vae.config.scaling_factor) + self.vae.config.shift_factor
            composite_tensor = self.vae.decode(composite_latents).sample
            foreground_latents = (foreground_latents / trans_vae.config.scaling_factor) + trans_vae.config.shift_factor
            foreground_tensor = trans_vae.decode(foreground_latents).sample

            # ---------------------------------------------------------------------------------------------
            composite_image = self.image_processor.postprocess(composite_tensor, output_type=output_type)[0]
            foreground_image = self.image_processor.postprocess(foreground_tensor, output_type=output_type)[0]
            image = [composite_image, foreground_image]
